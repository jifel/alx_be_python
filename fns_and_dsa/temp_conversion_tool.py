...
#implementing conversion fdactors

def convert_to_celsius(fahrenheit):
    celsius_conversion = (fahrenheit - 32) * FAHRENHEIT_TO_CELSIUS_FACTOR
...
confirm_temperature = input("Is this temperature in Celsius or Fahrenheit? (C/F): ").upper()

if confirm_temperature == "C":

    converted = convert_to_fahrenheit(user_temperature)

    print(f"{user_temperature}°C is {converted:1f}°F ")
        


elif confirm_temperature == "F":

    converted = convert_to_celsius(user_temperature)

    print(f"{user_temperature}°F is {converted:1f}°C")
